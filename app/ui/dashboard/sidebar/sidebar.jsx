...
const Sidebar = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const checkIdleBuses = (snapshotDocs) => {
        const now = new Date();
        const idleBuses = snapshotDocs.filter((doc) => {
          const data = doc.data();
          const driver = driversMap[data.driverID];
      ...
      const busLocationRef = collection(db, "BusLocation");

      // Initial load
      const initialSnapshot = await getDocs(busLocationRef);
      checkIdleBuses(initialSnapshot.docs);

      // Real-time updates
      const unsubIdle = onSnapshot(busLocationRef, (snapshot) => {
        checkIdleBuses(snapshot.docs);
      });
